...
class Program
{
  ...
  // i F F F 6 6 6 0 0 0 0 0 0 0 x x x x x x << this row last.
  // s A A A A 6 6 6 3 3 1 1 1 1 0 0 0 0 0 x << this row second.
  ...
  // Start at the s_i one to the left of the last 0 of the row before...
  static string CountStringSubsequenceDynamic(string source, string target)
  {
    ...
    int maxTargetIndex = target.Length - 1;
    int maxSourceIndex = source.Length - 1;

    // Record the position of the last nonzero match from the previous row
    int lastMatchedIndex = maxSourceIndex;
    for (int t_i = maxTargetIndex; t_i >= 0; t_i -= 1) {
      bool foundMatch = false;
      for (int s_i = lastMatchedIndex; s_i >= 0; s_i -= 1) {
        bool atRightEdge = s_i == maxSourceIndex;

        // If we don't have a match, we're the same value as the one to our right
        if (source[s_i] != target[t_i]) {
          if (!atRightEdge) {
            counts[t_i, s_i] = counts[t_i, s_i + 1];
          }
        }
        else {

          // Record matches so the next row can start one before this.
          if (!foundMatch) {
            foundMatch = true;
            lastMatchedIndex = s_i - 1;
          }

          // Either record 1 or 1 + the right one or the right one + diagonally down and right.
          bool atBottom = t_i == maxTargetIndex;
          if (atBottom) {
            counts[t_i, s_i] = atRightEdge ? 1 : counts[t_i, s_i + 1] + 1;
          }
          else {
            // If we're not at the bottom, we can't also be at the right edge because
            // the previous row MUST have found a match so lastMatchedIndex is at MOST
            // source.Length - 1 - 1.
            counts[t_i, s_i] = counts[t_i, s_i + 1] + counts[t_i + 1, s_i + 1];
          }
        }
      }

      if (!foundMatch) return "0000";
    }
